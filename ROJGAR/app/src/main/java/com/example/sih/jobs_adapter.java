...
public class jobs_adapter extends RecyclerView.Adapter<jobs_adapter.MyViewHolder> implements Filterable {
    Context context;
    ArrayList<job_data> details;
    ArrayList<job_data> fullDetails;
    Translate translate;
    String check, C;
    int d;

    public jobs_adapter(Context c, ArrayList<job_data> d){
        context = c;
        details = d;
        fullDetails = new ArrayList<>(d);
    }

    public jobs_adapter(Government c, ArrayList<String> content) {
    }

    @NonNull
    @Override
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new MyViewHolder(LayoutInflater.from(context).inflate(R.layout.jobs_cardview, parent, false));
    }
...
//            Picasso.get().load(details.get(position).getCompany_logo()).into(holder.company_logo);
    ...
    private Filter detailsFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ArrayList<job_data> filteredList = new ArrayList<>();
